...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireBaseScript : MonoBehaviour
    {
        ...
        private static FireBaseScript fireBaseScriptInstance;
        [SerializeField] private const float seconds = 2f;
        private static GameObject[] explodedEnemies;
        ...
        protected virtual void Awake()
        {
            ...
            fireBaseScriptInstance = this;
            explodedEnemies = new GameObject[0];
        }
        ...
        public static void CreateExplosion(Vector3 pos, float radius, float force)
        {
            ...
            {
                Rigidbody r = @object.GetComponent<Rigidbody>();
                if (r != null)
                {
                    r.AddExplosionForce(force, pos, radius);
                }
                if (@object.gameObject.tag == "Enemy" || @object.gameObject.tag == "FastEnemy" || @object.gameObject.tag == "SlowEnemy")
                {
                    
                    Array.Resize(ref explodedEnemies, explodedEnemies.Length + 1);
                    explodedEnemies[enemiesHit] = @object.gameObject;
                    enemiesHit++;
                }
            }
            foreach(GameObject enemy in explodedEnemies)
            {
                fireBaseScriptInstance.StartCoroutine(damageDelay(enemy.gameObject));
            }
        }
        ...
        private static IEnumerator damageDelay(GameObject enemy)
        {
